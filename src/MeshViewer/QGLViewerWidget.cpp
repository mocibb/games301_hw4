...
const acamcad::MVector3& QGLViewerWidget::Center(void) const
{
	return center;
}
...
void QGLViewerWidget::wheelEvent(QWheelEvent* _event)
{
	...
	Translate(acamcad::MVector3(0.0, 0.0, d));
	update();
}
...
void QGLViewerWidget::Translation(const QPoint & p)
{
	...
	Translate(acamcad::MVector3(2.0*dx / w*right / near_plane*z,
		-2.0*dy / h*top / near_plane*z,
		0.0f));
}

void QGLViewerWidget::Translate(const acamcad::MVector3 & _trans)
{
...
{
	acamcad::MVector3  newPoint3D;
	bool newPoint_hitSphere = MapToSphere(p, newPoint3D);
	if (newPoint_hitSphere)
	{
		acamcad::MVector3 axis = lastpoint3 % newPoint3D;
		if (axis.normSq() < 1e-7)
		{
			axis = acamcad::MVector3(1, 0, 0);
		}
		else
		{
			axis.normalize();
		...
		acamcad::MVector3 d = lastpoint3 - newPoint3D;
...
void QGLViewerWidget::Rotate(const acamcad::MVector3& _axis, const double & _angle)
{
	// Rotate around center center_, axis _axis, by angle _angle
	// Update modelview_matrix_

	acamcad::MVector3 t(modelviewmatrix[0] * center[0] +
...
bool QGLViewerWidget::MapToSphere(const QPoint& _v2D, acamcad::MVector3& _v3D)
{
...
{
	acamcad::MVector3 _trans = acamcad::MVector3(-(modelviewmatrix[0] * center[0] +
		modelviewmatrix[4] * center[1] +
		modelviewmatrix[8] * center[2] +
		modelviewmatrix[12]),
		-(modelviewmatrix[1] * center[0] +
