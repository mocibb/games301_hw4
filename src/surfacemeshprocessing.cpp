...
void SurfaceMeshProcessing::Open(void)
{
	viewer->Open();
	actReparam->setChecked(false);
	actReparam2->setChecked(false);
}

void SurfaceMeshProcessing::Reparam(bool b)
{
	std::cout << "SurfaceMeshProcessing::Reparam" << std::endl;
	actReparam2->setChecked(false);
	viewer->ShowReparam(b);
}

void SurfaceMeshProcessing::Reparam2(bool b)
{
	std::cout << "SurfaceMeshProcessing::Reparam2" << std::endl;
	actReparam->setChecked(false);
	viewer->ShowReparam2(b);
}
...
void SurfaceMeshProcessing::CreateActions(void)
{
	...
	actReparam = new QAction("Show Uniform Reparam", this);
	actReparam->setIcon(QIcon(":/SurfaceMeshProcessing/Images/Reparam.png"));
	actReparam->setStatusTip(tr("Show Uniform Reparam"));
	actReparam->setCheckable(true);
	// connect(actReparam, SIGNAL(toggled(bool)), viewer, SLOT(ShowReparam(bool)));
	connect(actReparam, SIGNAL(triggered(bool)), this, SLOT(Reparam(bool)));

	actReparam2 = new QAction("Show Floater Reparam", this);
	actReparam2->setIcon(QIcon(":/SurfaceMeshProcessing/Images/Reparam2.png"));
	actReparam2->setStatusTip(tr("Show Floater Reparam"));
	actReparam2->setCheckable(true);
	// connect(actReparam2, SIGNAL(toggled(bool)), viewer, SLOT(ShowReparam2(bool)));
	connect(actReparam2, SIGNAL(triggered(bool)), this, SLOT(Reparam2(bool)));
...
void SurfaceMeshProcessing::CreateToolBars(void)
{
	...
	tbView->addAction(actReparam);
	tbView->addAction(actReparam2);
}
